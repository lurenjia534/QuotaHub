package com.lurenjia534.quotahub.ui.screens.explore
 ...
 *
 * 功能说明：
 * - 这是用户点击底部导航栏 "Explore" 后看到的屏幕
 * - 目前只是一个占位页面，显示 "Explore Screen" 文字
 * - 未来可以在这里实现搜索、发现等功能
 *
 * 设计考量：
 * - 使用 Box + Alignment.Center 实现内容居中
 * - 使用 fillMaxSize() 让 Box 填满父容器全部空间
 * - 与 HomeScreen、ProfileScreen 保持一致的代码结构
 *
 ...
 *
 * 扩展建议：
 * 未来可以将 modifier 传递给 Box 之后，
 * 在 Box 内部添加更多 UI 组件，如：
 * - SearchBar（搜索栏）
 * - LazyColumn（可滚动的列表）
 * - 分类标签等
 */
