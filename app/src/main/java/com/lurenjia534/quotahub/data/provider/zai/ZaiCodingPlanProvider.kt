...
class ZaiCodingPlanProvider(
        ...
        return runCatching {
            ...
            CapturedQuotaSnapshot(
                ...
                replayPayload = ProviderReplayPayload(
                    fetchedAt = fetchedAt,
                    payloadFormat = RAW_PAYLOAD_FORMAT,
                    rawPayloadJson = json.encodeToString(ZaiUsageBundle.serializer(), bundle),
                    normalizerVersion = NORMALIZER_VERSION
                )
            )
        }
    ...
    companion object {
        ...
        private fun usageWindow(clock: Clock): ZaiUsageWindow {
...
private data class ZaiUsageWindow(
