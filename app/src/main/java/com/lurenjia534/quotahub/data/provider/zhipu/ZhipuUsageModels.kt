...
@Serializable
data class ZhipuUsageBundle(
    val modelUsage: ZhipuApiEnvelope<ZhipuModelUsageData>,
    val toolUsage: ZhipuApiEnvelope<ZhipuToolUsageData>,
    val quotaLimit: ZhipuApiEnvelope<ZhipuQuotaLimitData>
)

@Serializable
data class ZhipuModelUsageData(
    @SerialName("x_time")
    val xTime: List<String> = emptyList(),
    val modelCallCount: List<Long> = emptyList(),
    val tokensUsage: List<Long> = emptyList(),
    val totalUsage: ZhipuModelTotalUsage = ZhipuModelTotalUsage(),
    val modelDataList: List<ZhipuModelData> = emptyList(),
    val modelSummaryList: List<ZhipuModelSummary> = emptyList(),
    val granularity: String? = null
)

@Serializable
data class ZhipuModelTotalUsage(
...
@Serializable
data class ZhipuToolUsageData(
    @SerialName("x_time")
    val xTime: List<String> = emptyList(),
    val networkSearchCount: List<Long> = emptyList(),
    val webReadMcpCount: List<Long> = emptyList(),
    val zreadMcpCount: List<Long> = emptyList(),
    val totalUsage: ZhipuToolTotalUsage = ZhipuToolTotalUsage(),
    val toolDataList: List<ZhipuToolData> = emptyList(),
    val toolSummaryList: List<ZhipuToolSummary> = emptyList(),
    val granularity: String? = null
)

@Serializable
data class ZhipuToolTotalUsage(
    ...
    val toolDetails: List<ZhipuToolSummary> = emptyList(),
    val toolSummaryList: List<ZhipuToolSummary> = emptyList()
)

@Serializable
data class ZhipuToolData(
...
@Serializable
data class ZhipuQuotaLimitData(
    val limits: List<ZhipuQuotaLimit> = emptyList(),
    val level: String? = null
)

@Serializable
data class ZhipuQuotaLimit(
    val type: String,
    val unit: Int? = null,
    val number: Int? = null,
    val usage: Long? = null,
    ...
    val usageDetails: List<ZhipuQuotaUsageDetail> = emptyList()
)

@Serializable
data class ZhipuQuotaUsageDetail(
    val modelCode: String,
    val usage: Long = 0L
)

fun ZhipuUsageBundle.toQuotaSnapshot(
...
private fun ZhipuModelUsageData.toModelResources(
    sampleWindow: ZhipuSampleWindow?
): List<QuotaResource> {
    val summaries = when {
...
private fun ZhipuToolUsageData.toSampledToolResources(
    sampleWindow: ZhipuSampleWindow?
): List<QuotaResource> {
...
private fun ZhipuQuotaLimit.toLimitResource(
    index: Int,
    zoneId: ZoneId
): QuotaResource? {
...
private fun ZhipuQuotaLimit.toLimitDetailResources(
    zoneId: ZoneId
): List<QuotaResource> {
    val resetAt = nextResetTime?.takeIf { it > 0L }
    return usageDetails
...
private fun ZhipuModelUsageData.sampleWindow(
    zoneId: ZoneId
): ZhipuSampleWindow? {
    return xTime.toSampleWindow(zoneId)
}

private fun ZhipuToolUsageData.sampleWindow(
    zoneId: ZoneId
): ZhipuSampleWindow? {
    return xTime.toSampleWindow(zoneId)
}

private fun List<String>.toSampleWindow(zoneId: ZoneId): ZhipuSampleWindow? {
...
private fun ZhipuQuotaLimit.windowScope(): WindowScope {
    return when (unit) {
...
private fun ZhipuQuotaLimit.windowLabel(): String {
...
private fun ZhipuQuotaLimit.windowStartEpochMillis(
...
private fun <T> ZhipuApiEnvelope<T>.requireData(endpointName: String): T {
...
private fun ZhipuToolUsageData.summarizedToolUsage(vararg keys: String): Long {
...
private data class ZhipuSampledToolMetric(
    val key: String,
    val title: String,
    val usage: Long
)

private data class ZhipuSampleWindow(
    val startsAt: Long,
    val endsAt: Long
)
